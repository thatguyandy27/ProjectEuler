...
namespace ProjectEuler
{
    ...
    public class Problem54{
        ...
        public int GetPlayerOneWins(){
            var deals =  HandData.Split('/');
            int winCount = 0;

            foreach (var deal in deals)
            {
                var cards = GetCardsFromDeal(deal);

                var player1Cards = cards.Take(5).OrderBy(x => x.Value).ToArray();
                var player2Cards = cards.Reverse().Take(5).OrderBy(x => x.Value).ToArray(); 
                
                PokerRank player1Rank = GetHandRank(player1Cards),
                    player2Rank = GetHandRank(player2Cards);
                ...
                if (player1Rank > player2Rank){
                    ...
                    if (DoesPlayer1WinTie(player1Rank, player1Cards, player2Cards)){
                        winCount += 1;
                        // Console.WriteLine("player 1 wins tie ...");
                    }
                    // else {
                    //     Console.WriteLine("tie or player 2 wins...");
        ...
        private bool DoesPlayer1WinTie(PokerRank rank, Card[] player1Cards, Card[] player2Cards){
            switch (rank)
            {
                // if first card is bigger then you winna 
                case PokerRank.Straight:
                case PokerRank.StraightFlush:
                    return player1Cards[0].Value > player2Cards[0].Value; 
                
                case PokerRank.Flush:
                case PokerRank.HighCard:
                    for(int i = 4; i >= 0; i--){
                        if (player1Cards[i].Value != player2Cards[i].Value){
                            // values not equal there is a high card
                            return player1Cards[i].Value > player2Cards[i].Value; 
                        }
                    }
                    return false; // tie
                ...
                case PokerRank.FullHouse:
                    var player1CardCounts = GetCardCounts(player1Cards).OrderByDescending(x => x.Value).ThenByDescending(x => x.Key).ToList();
                    var player2CardCounts = GetCardCounts(player2Cards).OrderByDescending(x => x.Value).ThenByDescending(x => x.Key).ToList();

                    // we now have orderd by VALUE (# of cards) they by KEY (value of cards)
                    // iterate through each and check each the KEY (value) of each
                    for (int i = 0; i < player1CardCounts.Count; i++)
                    {
                        // Console.WriteLine("Player 1: {0}, Player 2: {1}", 
                            // player1CardCounts[i].Key, 
                            // player2CardCounts[i].Key);
                        if (player1CardCounts[i].Key != player2CardCounts[i].Key){
                            return player1CardCounts[i].Key > player2CardCounts[i].Key;
                        }
                    }
                    return false;

                // whateva we both winnas
        ...
        private Card[] GetCardsFromDeal(string deal){
            var rawCards = deal.Split(' ');
            ...
            foreach (var card in rawCards)
            {
                ...
                if (!int.TryParse(card.Substring(0, 1), out rank)){
                    switch(card[0]){
                        case 'T':
                            rank = 10;
                            break;
        ...
        // assume sorted by rank
        private PokerRank GetHandRank(Card[] sortedCards){
        
            bool isFlush = this.IsFlush(sortedCards),
                isStraight = this.IsStraight(sortedCards);

            if (isFlush && isStraight){

                // Is first card a jack
                if (sortedCards[0].Value == 11){
                ...
                var counts = GetCardCounts(sortedCards);
                if (counts.Any(x => x.Value == 4)){
                    return PokerRank.FourOfAKind;
                }

                if (counts.Any(x => x.Value == 3)){

                    if (counts.Any(x => x.Value == 2)){
                    ...
                    var count = counts.Count( x => x.Value == 2);
                    if (count == 2){
        ...
        private Dictionary<int, int> GetCardCounts(Card[] cards){
            Dictionary<int, int> counts = new Dictionary<int,int>();

            for (int i = 0; i < 5; i++)
            {
                var cardValue = cards[i].Value;
                if (counts.ContainsKey(cardValue)){
                    counts[cardValue]+=1;
                }
                else {
                    counts.Add(cardValue, 1);
                }
            }
        ...
        private bool IsStraight(Card[] cards){
            var currentCardValue = cards[0].Value;

            for (int i = 1; i < 5; i++)
            {
                currentCardValue++;
                if (cards[i].Value != currentCardValue){
                    return false;
                }
            }
        ...
        private bool IsFlush(Card[] cards){
            var card1Suit = cards[0].Suit;

            for (int i = 1; i < 5; i++)
            {
                if (card1Suit != cards[i].Suit){
                    return false;
                }
            }
        ...
        private const string HandData = "8C TS KC 9H 4S 7D 2S 5D 3S AC/" +  
